...
describe("pow2.Rect",()=> {
   it("should be defined", ()=> {
      expect(pow2.Rect).toBeDefined();
   });

   describe('constructor', ()=> {
      it("should initialize to 0,0,1,1 without arguments", ()=> {
         var r1:pow2.Rect = new pow2.Rect();
      ...
      it("should initialize from another rect instance",()=>{
         var r1:pow2.Rect = new pow2.Rect(15,15,0,0);
         var r2:pow2.Rect = new pow2.Rect(r1);
      ...
      it("should initialize from two points",()=>{
         var point:pow2.Point = new pow2.Point(0,0);
         var extent:pow2.Point = new pow2.Point(15,15);
         var r1:pow2.Rect = new pow2.Rect(point,extent);
         expect(r1.point).toEqual(point);
         expect(r1.extent).toEqual(extent);
      });

      it('should throw error with invalid arguments',()=>{
         var p:pow2.Rect = new pow2.Rect();
   ...
   describe('toString',()=>{
      it('should return "x,y,width,height" as a string',()=>{
         var p:pow2.Rect = new pow2.Rect(15.5,2.3,1,1);
   ...
   describe('set', ()=> {
      ...
      it("should accept two points as arguments",()=>{
         var point:pow2.Point = new pow2.Point(0,0);
         var extent:pow2.Point = new pow2.Point(15,15);
         var r1:pow2.Rect = new pow2.Rect().set(point,extent);
         expect(r1.point).toEqual(point);
         expect(r1.extent).toEqual(extent);
      });
   });
   ...
   describe('clone',()=>{
      it('should return a new instance with the identical values',()=>{
         var r1:pow2.Rect = new pow2.Rect(2,2,2,2);
         var r2:pow2.Rect = r1.clone();
   ...
   describe('clip',()=>{
      it('should clip this to be contained inside the given rectangle',()=>{
         var r1:pow2.Rect = new pow2.Rect(0,0,10,10);
         var r2:pow2.Rect = new pow2.Rect(-2,-2,4,4);
         var clipped:pow2.Rect = r2.clone();

         // Clip the point, adjusting the extent properly
         clipped.clip(r1);
         console.log(clipped.toString());
         expect(clipped.point.equal(new pow2.Point(0,0))).toBeTruthy();
         expect(clipped.extent.equal(new pow2.Point(2,2))).toBeTruthy();
      });
   });

   describe('isValid',()=>{
      it('should return true when extent is greater than zero',()=>{
         expect(new pow2.Rect(0,0,-1,-1).isValid()).toBeFalsy();
         expect(new pow2.Rect(0,0,0,0).isValid()).toBeFalsy();
         expect(new pow2.Rect(0,0,1,1).isValid()).toBeTruthy();
      });
   });
   ...
   describe('intersect',()=>{
      it('should return true if the two rectangles overlap at all',()=>{
         var r1:pow2.Rect = new pow2.Rect(0,0,10,10);
         var r2:pow2.Rect = new pow2.Rect(-2,-2,4,4);
         var r3:pow2.Rect = new pow2.Rect(-6,-6,5,5);
   ...
   describe('pointInRect',()=>{
      ...
      it('should return false if a point is not contained within the rectangle',()=>{
         var r1:pow2.Rect = new pow2.Rect(0,0,10,10);
         expect(r1.pointInRect(new pow2.Point(-2,-2))).toBeFalsy();
         expect(r1.pointInRect(new pow2.Point(12,12))).toBeFalsy();
      });
      ...
      it('should throw error with invalid arguments',()=>{
         var r1:pow2.Rect = new pow2.Rect(0,0,10,10);
         expect(() => { r1.pointInRect(<any>'four',<any>10); }).toThrow(new Error(pow2.errors.INVALID_ARGUMENTS));
      });
   ...
   describe('getCenter',()=>{
      it('should return the center point of the rectangle',()=>{
         var r1:pow2.Rect = new pow2.Rect(0,0,10,5);
         expect(r1.getCenter().equal(new pow2.Point(5,2.5))).toBeTruthy();
      });
   });
   ...
   describe('setCenter',()=>{
      it('should set the center point for a rectangle',()=>{
         var r1:pow2.Rect = new pow2.Rect(0,0,10,10);
         var r2:pow2.Rect = r1.clone().setCenter(5,5);
      ...
      it('should accept a point instance as argument',()=>{
         var r1:pow2.Rect = new pow2.Rect(0,0,10,10);
         // positive 5 + -(10/2) = 0
         r1.setCenter(new pow2.Point(5,5));
      ...
      it('should accept x and y as number arguments',()=>{
         ...
         r1.setCenter(-5,-5);
         expect(r1.point.equal(new pow2.Point(-10,-10))).toBeTruthy();
      });
   });

   describe('getLeft',()=>{
      it('should return the point of the left edge',()=>{
         expect(new pow2.Rect(5,5,5,5).getLeft()).toBe(5);
      });
   });
   describe('getTop',()=>{
      it('should return the point of the top edge',()=>{
         expect(new pow2.Rect(5,5,5,5).getTop()).toBe(5);
      });
   });
   describe('getRight',()=>{
      it('should return the point of the right edge',()=>{
         expect(new pow2.Rect(5,5,5,5).getRight()).toBe(10);
      });
   });
   describe('getBottom',()=>{
      it('should return the point of the bottom edge',()=>{
         expect(new pow2.Rect(5,5,5,5).getBottom()).toBe(10);
      });
   });

   describe('getHalfSize',()=>{
      it('should return half of the extent of the rectangle',()=>{
         expect(new pow2.Rect(5,5,10,10).getHalfSize().equal(new pow2.Point(5,5))).toBeTruthy();
         expect(new pow2.Rect(0,0,9,9).getHalfSize().equal(new pow2.Point(4.5,4.5))).toBeTruthy();
      });
   });

   describe('addPoint',()=>{
      it('should expand rectangle to contain point',()=>{
         var r1:pow2.Rect = new pow2.Rect(0,0,1,1);
         r1.addPoint(new pow2.Point(2,2));
         expect(r1.extent.equal(new pow2.Point(2,2))).toBeTruthy();
         expect(r1.point.isZero()).toBeTruthy();

         r1.addPoint(new pow2.Point(-2,-2));
         expect(r1.extent.equal(new pow2.Point(4,4))).toBeTruthy();
         expect(r1.point.equal(new pow2.Point(-2,-2))).toBeTruthy();
      });
   });
   ...
   describe('inflate',()=>{
      it('should expand by the given x and y along each axis in both directions',()=>{
         ...
         r1.inflate(2,2);
         expect(r1.point.equal(new pow2.Point(-2,-2))).toBeTruthy();
         expect(r1.extent.equal(new pow2.Point(5,5))).toBeTruthy();
      });
      it('should expand by one unit along each axis in both directions if not specified',()=>{
         var r1:pow2.Rect = new pow2.Rect(0,0,1,1);
         r1.inflate();
         expect(r1.point.equal(new pow2.Point(-1,-1))).toBeTruthy();
         expect(r1.extent.equal(new pow2.Point(3,3))).toBeTruthy();
      });
   });
});