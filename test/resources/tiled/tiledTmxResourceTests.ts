...
describe("pow2.TiledTMXResource", ()=> {
  ...
  it("should succeed with good url", (done)=> {
    ...
    var resource = loader.load<pow2.TiledTMXResource>('base/test/fixtures/example.tmx');
  ...
  it("should fail with bad url", (done)=> {
    var loader:pow2.ResourceLoader = new pow2.ResourceLoader();
    var resource = loader.load<pow2.TiledTMXResource>('bad/does/not/exist.tmx');
  ...
  it("should fail with missing image source", (done)=> {
    var loader:pow2.ResourceLoader = new pow2.ResourceLoader();
    var resource = loader.load<pow2.TiledTMXResource>('base/test/fixtures/badImage.tmx');
    resource.on(pow2.Resource.FAILED, ()=> {
      done();
    });
  });
  ...
  it("should fail with non-csv encoded layer data", (done)=> {
    var loader:pow2.ResourceLoader = new pow2.ResourceLoader();
    var resource = loader.load<pow2.TiledTMXResource>('base/test/fixtures/badEncoding.tmx');
    resource.on(pow2.Resource.FAILED, ()=> {
      done();
    });
  });
